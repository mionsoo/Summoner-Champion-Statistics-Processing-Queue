...
class RiotV4Tier(RiotV4):
    ...
    def get_cgm_url(self, queue_type, tier):
        if tier == "challenger":
            target_url = (
                f"https://{self.platform_id}.api.riotgames.com/lol/league/v4/challengerleagues/by-queue/"
                f"{queue_type}?api_key={self.__api_key}"
            )
        elif tier == "grandmaster":
            target_url = (
                f"https://{self.platform_id}.api.riotgames.com/lol/league/v4/grandmasterleagues/by-queue/"
                f"{queue_type}?api_key={self.__api_key}"
            )
        else:  # tier == 'master'
            target_url = (
                f"https://{self.platform_id}.api.riotgames.com/lol/league/v4/masterleagues/by-queue/"
                f"{queue_type}?api_key={self.__api_key}"
            )
        return target_url
    ...
    def get_url(self, queue_type, tier, division, page):
        if page:
            target_url = (
                f"https://{self.platform_id}.api.riotgames.com/lol/league/v4/entries/"
...
class RiotV4Summoner(RiotV4):
    ...
    def get_url(self, summoner_name=False, summoner_id=False, puu_id=False):
        if summoner_name:
            target_url = (
                f"https://{self.platform_id}.api.riotgames.com/lol/summoner/v4/summoners/by-name/"
                f"{summoner_name}?api_key={self.__api_key}"
            )
        elif summoner_id:
            target_url = (
                f"https://{self.platform_id}.api.riotgames.com/lol/summoner/v4/summoners/"
                f"{summoner_id}?api_key={self.__api_key}"
            )
        elif puu_id:
            target_url = (
                f"https://{self.platform_id}.api.riotgames.com/lol/summoner/v4/summoners/by-puuid/"
...
class RiotV5Match(RiotV5):
    ...
    def get_match_ids_url(self, queue_type, start_time=False, end_time=False, start_idx=0, count=10):
        ...
        if queue_id == 100:
            target_url = (
                f"https://{self.region_v5}.api.riotgames.com/lol/match/v5/matches/by-puuid/"
                f"{self.puu_id}/ids?"
        ...
        if start_time and end_time:
            if isinstance(type(start_time), int) and isinstance(type(end_time), int):
                ...
                raise TypeError(
                    "start_time과 end_time을 입력할 때는 integer timestamp (ex. 6124551123) 형태로 입력 필요"
                )
    ...
    def get_match_url(self, match_id, target_type):
        if target_type == "match":
            target_url = (
                f"https://{self.region_v5}.api.riotgames.com/lol/match/v5/matches/"
...
class RiotV4Spectator(RiotV4):
    ...
    def get_url(self, summoner_id=False):
        target_url = (
            f"https://{self.platform_id}.api.riotgames.com/lol/spectator/v4/active-games/by-summoner/"
            f"{summoner_id}?api_key={self.__api_key}"
        )
...
class RiotV4ChampionMastery(RiotV4):
    ...
    def get_url(self, summoner_id: str, champion_id: int):
        target_url = (
            f"https://{self.platform_id}.api.riotgames.com/lol/champion-mastery/v4/champion-masteries/"
...
class RiotV1Challenges(RiotV4):
    ...
    def get_url(self, puu_id: str):
        target_url = (
            f"https://{self.platform_id}.api.riotgames.com/lol/challenges/v1/player-data/"
            f"{puu_id}?api_key={self.__api_key}"
        )
...
class RiotV1Accounts:
    ...
    def get_url_by_puu_id(self, puu_id):
        target_url = (
            f"https://asia.api.riotgames.com/riot/account/v1/accounts/by-puuid/{puu_id}?api_key={self.__api_key}"
        )
        return target_url
