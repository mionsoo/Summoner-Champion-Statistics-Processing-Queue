...
def insert_summoner_basic_info(
    res: dict,
    platform_id: str,
) -> bool:
    """
    ...
    try:
        riot_id_name = (
            copy.deepcopy(res["riot_id_name"]).replace("%20", "").replace(" ", "").replace("\xa0", "").lower()
        )
        ...
        wins_flex, losses_flex, mini_progress_flex, mini_wins_flex, mini_losses_flex = get_ranked_win_loss(
            res=res, ranked="RANKED_FLEX_SR"
        )
        ...
        rank_flex = (
            tier_flex
            if tier_flex in ("CHALLENGER", "GRANDMASTER", "MASTER", "unranked")
            else f"{tier_flex} {division_flex}"
        )
...
def is_unsearchable_response(result):
    return (
        ...
        or result.status_code == 403
        or result.status_code == 404
        or result.status_code == 401
        or result.status_code == 503
    )
